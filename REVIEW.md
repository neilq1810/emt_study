...
| 25 | Error taxonomy & budgets | 🟢 DEEPENED → review | +GUM law of propagation (eq 25.1, Type A/B, combined/expanded uncertainty), **worked numeric position budget** (→0.84 mm @95%, top-down allocation), correlation cross-terms (1696→2411 w) | **+§25.3 item 7 nonlinearity → harmonic/IMD distortion**: sensor (cored-coil saturation, MR/TMR nonlinear curve) + AFE THD/IP3 + ADC INL + drive THD; amplitude-compression bias + harmonic folding into FDM channels; the key point — it is SIGNAL-AMPLITUDE-DEPENDENT (signal swings ~1/r³) so a near-generator transfer-curve cal mis-corrects at the edge → a *pose-dependent* bias needing 2-D cal; harmonic content doubles as a saturation flag (Ch.27). +budget-table row. **+§25.3 items 8–11** (the field-sensor-centric deterministic gaps): cross-axis/transverse sensitivity & non-orthogonality (off-diagonal sensor matrix); sensor offset & offset drift (coil has none; AC & pulsed-DC reject static offset); hysteresis (history-dependent bias that breaks the calibratable-bias assumption); amplifier gain/offset (gain=mult. bias, offset lock-in-rejected). +coil-vs-field-sensor framing note (incl. magnetostriction, DC geomagnetic background, overload/clipping); 4 budget rows. (voltage/current noise was already §25.2 item 4.) **+§25.8 layered deep dive (source→impact→mitigation, by scheme & sensor)**: a per-source template + **Matrix A** (error × modulation: sine / triangle→square / pulsed-DC) + **Matrix B** (error × sensor: air coil / ferrite coil / TMR), with **a full deep entry per source** (12: nonlinearity, field distortion, stochastic noise floor, offset/drift, channel crosstalk, thermal drift, generator/drive noise, motion & latency skew, cross-axis/non-orthogonality, hysteresis, RTS/popcorn, cable/connector microphonic — each on the Source→Impact→Mitigation→Modulation-dependence→Sensor-dependence template). Centerpiece is the **sim-19-backed nonlinearity×waveform result** (`figures/ch25_nonlinearity_waveform.png`): a cubic makes a ~1.3% third harmonic that a sine lock-in **rejects out of band** but a square/triangle demod **sums in-band**; with multiple tones (or the square's own harmonic comb) the 3rd-order IMD lands in-band on other channels (~9% of a tone vs ~0% single-tone) — confirming a harmonic-rich excitation makes nonlinearity materially harder to manage, worst-paired with a TMR sensor. Contract assertion #16 added. [user-requested: significantly more depth + layered structure]
